...
export default function ModifierFournisseurPage() {
  ...
  // 🔥 Charger le fournisseur et les domaines
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resFournisseur, resDomaines] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/fournisseur/${id}`),
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/domaine`)
        ]);

        const jsonFournisseur = await resFournisseur.json();
        const jsonDomaines = await resDomaines.json();

        if (!resFournisseur.ok) throw new Error(jsonFournisseur.message || "Impossible de charger le fournisseur");
        if (!resDomaines.ok) throw new Error(jsonDomaines.message || "Impossible de charger les domaines");

        setName(jsonFournisseur.data.name);
        setAddress(jsonFournisseur.data.address || "");
        setEmail(jsonFournisseur.data.email || "");
        setContact(jsonFournisseur.data.contact || "");
        setSelectedDomaines(jsonFournisseur.data.domaines.map(d => d.id));
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !address || selectedDomaines.length === 0) {
      setError("Veuillez remplir tous les champs obligatoires et sélectionner au moins un domaine");
      setSaving(false);
      return;
    }
    ...
    try {
      ...
      setMessage("Fournisseur modifié avec succès ✔️");
  ...
  return (
    <section className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Modifier fournisseur</h1>
        <p className="text-gray-700 text-lg">
          Mettez à jour les informations du fournisseur et ses domaines.
        </p>
      </div>
